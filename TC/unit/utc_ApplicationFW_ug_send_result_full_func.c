...
static Eina_Bool tst01(void *data)
{
	...
	r = ug_send_result_full(ug, result,APP_CONTROL_RESULT_SUCCEEDED);
	if (r) {
...
void result_cb(ui_gadget_h ug, app_control_h reply, void *priv)
{
...
static Eina_Bool tst02(void *data)
{
	...
	r = ug_send_result_full(NULL, NULL,APP_CONTROL_RESULT_SUCCEEDED);
	if (!r) {
	...
	r = ug_send_result_full(ug, NULL,APP_CONTROL_RESULT_SUCCEEDED);
	if (!r) {
		tet_infoline("ug_send_result() failed in negative test case");
		tet_result(TET_FAIL);
		goto exit;
	...
	r = ug_send_result_full(NULL, result,APP_CONTROL_RESULT_SUCCEEDED);
	if (!r) {
		tet_infoline("ug_send_result() failed in negative test case");
		tet_result(TET_FAIL);
		goto exit;
